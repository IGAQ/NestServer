...
export class UsersService implements IUsersService {
	...
	public async findUserByUsername(username: string): Promise<User | undefined> {
		const user = await this._neo4jService.read(`MATCH (u:User {username: $username}) RETURN u`, { username: username });
	...
	public async findUserById(userId: string): Promise<User | undefined> {
		const user = await this._neo4jService.read(`MATCH (u:User {userId: $userId}) RETURN u`, { userId: userId });
		if (user.records.length === 0) return undefined;
		let foundUser = user.records[0].get("u").properties;
		foundUser.role = foundUser.role.map(r => r.low) as Role[];
		return foundUser;
	...
	public async addUser(user: RegisterUserPayloadDto): Promise<void> {
		this._neo4jService.write(`CREATE (u:User {
			userId: $userId,
			createdAt: $createdAt,
			updatedAt: $updatedAt,
			email: $email,
	...
	public async updateUser(user: User): Promise<void> {
		this._neo4jService.write(`MATCH (u:User {userId: $userId}) 
		SET 
