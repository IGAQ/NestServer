...
export class CommentsService implements ICommentsService {
    ...
    public async findCommentById(commentId: string): Promise<Comment> {
        ...
        if (foundComment.pending) {
            throw new HttpException("Comment cannot be shown publicly due to striking policies", 403);
        }
    ...
    public async voteComment(voteCommentPayload: VoteCommentPayloadDto): Promise<void> {
        ...
        const comment = await this._dbContext.Comments.findCommentById(voteCommentPayload.commentId);
    ...
    public async markAsDeleted(commentId: string): Promise<void> {
        ...
        await comment.setDeletedProps(new DeletedProps({
